...
class TestFxGraphCache(TestCase):
    ...
    def test_graphsafe_rng_cache_hit(self):
        ...
        This HOP must be marked cacheable so FxGraphCache stores and
        reloads compiled code instead of bypassing.
        """
        ...
        model = checkpoint_wrapper(SDPADrop(), preserve_rng_state=True)
        ...
        # Verify the HOP is in the graph: without cacheable, bypass fires
        with patch.object(graphsafe_run_with_rng_state, "_cacheable", False):
            run(q)
            q.grad = None
            self.assertGreater(counters["inductor"]["fxgraph_cache_bypass"], 0)
