...
def batch_norm_backward_strategy(
    ...
    # Channel-dim sharding: grad_input [N,C,*] on dim 1, grad_weight/grad_bias [C] on dim 0.
    num_tensor_inputs = sum(isinstance(a, TensorMeta) for a in args_schema)
    rule: list[Placement | _ShardingPlaceholder] = [
    ...
    # remaining 1D tensor inputs: weight, running_mean/var, save_mean/invstd
...
# Register scalar shape adjuster for group_norm forward and backward so the
# sharding propagator rewrites the N/C/HxW args to local values when sharded.
# Both ops have the same layout: tensor args followed by N, C, HxW scalars,
# and the first tensor (input / grad_out) has shape [N, C, *].
